...
from resource.add_testpaper import Ui_Form
from mytool import *
from functools import partial

import time

class AddPaperTest(QDialog, Ui_Form):
    ...
    def zhuanye_changed(self):
        ...
        if len(self.tiku) > 0:
            for m in self.tiku:
                if self.tiku[m][3] in ls:
                    num_tiku[ls.index(self.tiku[m][3])] += 1  # 将数字取出来转换后变成下标
        else:
            print("题库中无信息")
    ...
    def on_clicked(self):
        ...
        if len(self.tiku) > 0:
            ...
            ls = ["单选题", "多选题", "判断题", "填空题", "问答题"]
            for x in self.tiku:
                mytk[ls.index(self.tiku[x][3])].append(x)
            ...
            QMessageBox.about(self, "生成试卷", "生成试卷成功！")
            self.close()
